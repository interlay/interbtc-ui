...
const handleLockTimeChange = (event: KeyboardEvent) => {
  if (event.key.length > 1 || integerRegexPattern.test(event.key)) return;
...
const LockTimeField = React.forwardRef<Ref, CustomProps & NumberInputProps>(
  ({ optional, id, name, error, helperText, ...rest }, ref): JSX.Element => {
    ...
    return (
      <div>
        ...
        <div className={clsx('flex', 'justify-between', 'items-center')}>
          ...
          {optional === false && <span className={LABEL_TEXT_COLOR_CLASSES}>Choose lock time in weeks:</span>}
          {optional === undefined && <span className={LABEL_TEXT_COLOR_CLASSES}>Checking...</span>}
          <div className={clsx('inline-flex', 'items-center', 'space-x-2.5')} ref={wrappingRef}>
            <NumberInput
              ref={ref}
              id={id}
              className={clsx('!w-12', {
              ...
              {...rest}
            />
            <span className={clsx('text-xs', LABEL_TEXT_COLOR_CLASSES)}>Weeks</span>
          </div>
        </div>
        <TextFieldHelperText
          className={clsx(
