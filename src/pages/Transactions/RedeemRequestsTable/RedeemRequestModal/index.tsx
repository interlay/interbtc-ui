...
const RedeemRequestModal = ({
  ...
  const renderModalStatusPanel = (request: any) => {
    switch (request.status) {
      case RedeemStatus.Expired: {
        return <ReimburseStatusUI request={request} onClose={onClose} />;
      }
      default: {
  ...
  const redeemedWrappedTokenAmount = request.request.requestedAmountBacking
    .add(request.bridgeFee)
    .add(request.btcTransferFee);
          ...
          <div className='space-y-6'>
            ...
            <div className='space-y-4'>
              <div className={clsx('flex', 'justify-between')}>
                ...
                <span className='font-medium'>{shortAddress(request.userBackingAddress || '')}</span>
              </div>
              <div className={clsx('flex', 'justify-between')}>
                <span
                  className={clsx(
                ...
                <span className='font-medium'>{request.request.height.active}</span>
              </div>
              <div className={clsx('flex', 'justify-between')}>
                <span
                  className={clsx(
                ...
                <span className='font-medium'>{shortAddress(request.vault.accountId || '')}</span>
              </div>
            </div>
          </div>
