...
import { APP_NAME, TERMS_AND_CONDITIONS_LINK } from 'config/relay-chains';
      ...
      return (
        <>
          {accounts !== undefined && accounts.length > 0 ? (
            ...
            <p>
              {t('no_account')}
              <ExternalLink href={POLKADOT_EXTENSION}>&nbsp;{t('here')}</ExternalLink>.
            </p>
          )}
        </>
      );
        ...
        <>
          ...
          <ExternalLink href={POLKADOT_EXTENSION}>
            <span className={clsx('inline-flex', 'items-center', 'space-x-1.5')}>
              <PolkadotExtensionLogoIcon width={30} height={30} />
    ...
    <InterlayModal initialFocus={focusRef} open={open} onClose={onClose}>
      <InterlayModalInnerWrapper className={clsx('p-6', 'max-w-lg')}>
        <InterlayModalTitle as='h3' className={clsx('text-lg', 'font-medium', 'mb-6')}>
          {extensionWalletAvailable ? 'Select account' : 'Pick a wallet'}
        </InterlayModalTitle>
        <CloseIconButton ref={focusRef} onClick={onClose} />
        <div className='space-y-4'>
          {renderContent()}
          <div className={clsx('flex', 'justify-end')}>
            <InterlayMulberryOutlinedButton onClick={onClose}>Close</InterlayMulberryOutlinedButton>
          </div>
        </div>
